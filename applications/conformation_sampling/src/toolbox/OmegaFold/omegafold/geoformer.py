...
class GeoFormerBlock(modules.OFModule):
    ...
    def forward(
        ...
        node_repr = node_repr + self.attention_w_edge_bias(
            node_repr,
            edge_repr,
            mask,
            fwd_cfg=fwd_cfg
        ...
        node_repr = node_repr + self.node_transition(
            node_repr,
            subbatch_size=fwd_cfg.subbatch_size
        )

        edge_repr = edge_repr + self.out_product(node_repr, mask)
        
        for layer in self.geometric_attention:
            edge_repr = edge_repr + layer(edge_repr, mask[..., 0, :], fwd_cfg=fwd_cfg)
        
        edge_repr = edge_repr + self.edge_transition(edge_repr, fwd_cfg.subbatch_size)
    ...
    def _column_attention(self, node_repr, mask, fwd_cfg):
        ...
        node_repr = node_repr+node_repr_col.transpose(-2, -3)
        return node_repr
