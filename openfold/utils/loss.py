...
def torsion_angle_loss(
    ...
    l_torsion = torch.sum(min_diff*mask, dim=(-1, -2)) / (torch.sum(mask, dim=(-1, -2)) + 1e-2)
    l_angle_norm = torch.sum(torch.abs(norm - 1), dim=(-1, -2)) / (torch.sum(mask, dim=(-1, -2)) + 1e-2)
