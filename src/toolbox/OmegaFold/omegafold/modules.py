...
def _attention(
    ...
    #logits.add_(bias)
    logits = logits + bias
    #attn = softmax(logits, dim=-1, in_place=not return_edge)#in_place=not return_edge
    attn = softmax(logits, dim=-1, in_place=False)
...
class AttentionWEdgeBias(OFModule):
    ...
    def forward(
        ...
        # tmp = self.proj_edge_bias(edge_repr)
        # # Check the input shape
        # if len(tmp.shape) == 4:
        #     # Permute for 4D tensor
        #     edge_bias = tmp.permute(0, 3, 1, 2)
        #     edge_bias = edge_bias[:,None,...] + utils.mask2bias(mask[..., None, None, :])
        # elif len(tmp.shape) == 3:
        #     # Permute for 3D tensor
        #     edge_bias = tmp.permute(2, 0, 1)
        #     edge_bias = edge_bias + utils.mask2bias(mask[..., None, None, :])
        # else:
...
class GeometricAttention(OFModule):
    ...
    def _get_attended(
        ...
        b = torch.zeros(
            # self.n_axis, self.n_head, *edge_repr.shape[:-1],
            self.n_axis, self.n_head, *edge_repr.shape[:2],
            dtype=edge_repr.dtype,
            device=edge_repr.device
        )
        #b += utils.mask2bias(mask)
        b = b + utils.mask2bias(mask)
        
        for s, e, edge_r in _get_sharded_stacked(edge_repr, subbatch_size=fwd_cfg.subbatch_size):
            # # print(edge_r.shape, self.linear_b_weights.shape,self.linear_b_bias.shape)
            # if len(edge_r.shape)==5:
            #     # print(edge_r.shape, self.linear_b_weights.shape,self.linear_b_bias[None,...].shape)
            #     # print(b.shape,s, e,fwd_cfg.subbatch_size,edge_repr.shape[-2])#+torch.Size([4, 2, 4, 105, 105])
            #     # print(edge_repr.shape[:-2])
            #     # print(edge_repr.shape,self.n_axis, self.n_head)
            #     # print(mask.shape)
            #     # exit()
            #     b[..., s:e, :] = torch.einsum(
            #     '...qkcr,crh->...rhqk', edge_r, self.linear_b_weights #TODO here
            # ) + self.linear_b_bias
            # else:
            #     b[..., s:e, :] = torch.einsum(
    ...
    def forward(
        ...
        out = out + self._get_gated(edge_repr, mask, fwd_cfg)
        return out
