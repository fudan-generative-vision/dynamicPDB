...
class InvariantPointAttention(modules.OFModule):
    ...
    def forward(
        ...
        # logits += utils.mask2bias(frames.mask[...,None, :, None])
        # attn_w = modules.softmax(logits, dim=-2, in_place=True)  # change here
...
class StructureCycle(modules.OFModule):
    ...
    def forward(
        ...
        """
        node_repr = node_repr + self.ipa(node_repr, edge_repr, backbone_frames)
        ...
        node_repr = node_repr+input_repr  # Shortcut residual connection
